(ns falx.engine.scene
  (:require [falx.engine.rect :as rect]
            [falx.engine.size :as size]
            [falx.engine.scene.protocols :refer [INode ISized -size -layout] :as proto])
...
(extend-protocol ISized
  falx.engine.scene.protocols.IWrap
  (-size [this rect]
    (size (proto/-child this) rect))
  falx.engine.scene.protocols.IWrapMany
  (-size [this rect]
...
(defrecord Fit [node size]
  ...
  proto/ISized
  (-size [this rect]
    (or size (falx.engine.scene/size node rect))))
...
(defn table*
  [head & rows]
  (apply falx.engine.scene/rows
         (coll->cols head)
         (map coll->cols rows)))

(defn htable*
  [head & rows]
  (apply falx.engine.scene/cols
         (coll->rows head)
         (map coll->rows rows)))
...
(defn maptable
  ...
  ([kvs opts]
   ...
   (let [keys (map first kvs)
         vals (map second kvs)]
     (if (:vertical? opts)
       (falx.engine.scene/rows
         (coll->cols keys)
         (coll->cols vals))
       (falx.engine.scene/cols
         (coll->rows keys)
         (coll->rows vals))))))
